...
class DailyReportView(APIView):
    def get(self, request, date_str):
        ...
        # AI摘要
        prompt = (
            "请根据以下监控数据，严格只输出结构化JSON摘要，字段包括：overview（整体概览，字符串，不要描述正常事件或正常数），type_summary（异常类型分布，数组，每项含type和desc），"
            "key_points（重点关注，数组），suggestions（建议，数组），所有内容用简洁正式中文。不要输出任何解释、markdown、注释，只要JSON。不要描述正常事件或正常数。\n"
