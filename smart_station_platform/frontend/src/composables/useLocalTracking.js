...
export function useLocalTracking() {
  ...
  onUnmounted(() => {
    ...
    if (model.value) {
      try {
        // 释放模型资源
        model.value.dispose(); // 释放模型
        model.value = null; // 将模型引用置为null

        // 清理TensorFlow内存
        if (tf && tf.getBackend()) { // 确保tf和后端都存在
          // 确保在有效的 TensorFlow.js 引擎上下文中调用 disposeVariables
          // tf.engine()._engineState.activeScope 是一个私有属性，不建议直接访问
          // 更好的做法是确保你所有的 tf.tensor 和 tf.variable 都被正确 dispose
          // 如果没有显式 beginScope/endScope，通常不需要调用 endScope
          tf.disposeVariables(); // 释放所有持久化变量

          // 如果你怀疑有未被 dispose 的张量，可以打印内存信息调试
          // console.log('TensorFlow memory after disposeVariables:', tf.memory());
        }
      } catch (e) {
        // 确保捕获到的错误信息有意义，并记录下来
